...
--
-- INSTRUCTIONS:
-- This file can be executed all at once in Supabase SQL Editor, or section by section.
...
-- Enable pg_trgm for fuzzy text search (optional, useful for search features)
...
-- ============================================================================
-- SECTION 3: IDENTITY & SOCIAL TABLES
-- ============================================================================
...
CREATE TABLE user_profiles (
    ...
    timezone TEXT,
    birth_year INTEGER,
    favorite_team_id UUID NULL, -- Will be linked after teams table is created
...
-- ============================================================================
-- SECTION 5: LIVE GAME DATA (needed for foreign keys)
-- ============================================================================
...
-- Now add the foreign key constraint for user_profiles.favorite_team_id
...
-- Drives table (football-specific)
CREATE TABLE drives (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ============================================================================
-- SECTION 6: OPERATIONS & ANALYTICS (needed for foreign keys)
-- ============================================================================

-- Content releases table
...
-- ============================================================================
-- SECTION 8: PROGRESSION & GAMIFICATION
-- ============================================================================
...
-- User XP events table (partitioned by month)
...
CREATE TABLE user_xp_events_2025_12 PARTITION OF user_xp_events
...
CREATE TABLE leaderboards (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    sport_id UUID NOT NULL REFERENCES sports(id) ON DELETE CASCADE,
    time_window leaderboard_window NOT NULL,
    ...
    xp INTEGER NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT leaderboards_unique_rank UNIQUE (sport_id, time_window, window_start, rank)
);

CREATE INDEX idx_leaderboards_sport_window ON leaderboards(sport_id, time_window, window_start, rank);
CREATE INDEX idx_leaderboards_user ON leaderboards(user_id, time_window);

COMMENT ON TABLE leaderboards IS 'Materialized leaderboard rankings (regenerated periodically)';


-- ============================================================================
-- SECTION 9: ASSESSMENT & TRACKING
-- ============================================================================

-- Provider events table (partitioned, needed for foreign keys)
...
CREATE TABLE provider_events_2025_11 PARTITION OF provider_events
    FOR VALUES FROM ('2025-11-01') TO ('2025-12-01');
CREATE TABLE provider_events_2025_12 PARTITION OF provider_events
    FOR VALUES FROM ('2025-12-01') TO ('2026-01-01');
...
CREATE TABLE submission_judgments (
    ...
    provider_event_received_at TIMESTAMPTZ NULL,
    confidence NUMERIC(3, 2),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    FOREIGN KEY (submission_id, submission_submitted_at) REFERENCES submissions(id, submitted_at) ON DELETE CASCADE,
    FOREIGN KEY (provider_event_id, provider_event_received_at) REFERENCES provider_events(id, received_at)
);
...
CREATE TABLE srs_reviews (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    FOREIGN KEY (submission_id, submission_submitted_at) REFERENCES submissions(id, submitted_at)
);
...
-- ============================================================================
-- SECTION 11: LIVE PROMPTING
-- ============================================================================

-- Live prompt mappings table
...
CREATE TABLE provider_mappings (
    ...
    provider_event_received_at TIMESTAMPTZ NOT NULL,
    play_id UUID NOT NULL REFERENCES plays(id),
    mapping_notes TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    FOREIGN KEY (provider_event_id, provider_event_received_at) REFERENCES provider_events(id, received_at) ON DELETE CASCADE
);
...
-- Materialized view: User leaderboard ranks
...
-- Function: Update overall rating
...
CREATE TRIGGER update_user_item_stats_updated_at BEFORE UPDATE ON user_item_stats
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_srs_cards_updated_at BEFORE UPDATE ON srs_cards
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_live_prompts_updated_at BEFORE UPDATE ON live_prompts
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_live_prompt_windows_updated_at BEFORE UPDATE ON live_prompt_windows
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
INSERT INTO leagues (sport_id, slug, name, country) VALUES
    ...
    ((SELECT id FROM sports WHERE slug = 'baseball'), 'mlb', 'MLB', 'USA'),
    ((SELECT id FROM sports WHERE slug = 'hockey'), 'nhl', 'NHL', 'USA/CAN'),
    ((SELECT id FROM sports WHERE slug = 'soccer'), 'mls', 'MLS', 'USA/CAN'),
    ((SELECT id FROM sports WHERE slug = 'golf'), 'pga', 'PGA Tour', 'USA');
...
SELECT
    'Sports seeded:' AS info,
    COUNT(*) AS count
FROM sports;

SELECT
    'Leagues seeded:' AS info,
    COUNT(*) AS count
FROM leagues;
